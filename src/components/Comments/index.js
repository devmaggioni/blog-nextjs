...
import {
  showReplyContainer,
  showContainerNewComment,
  showContainerNewReply,
  newDate,
} from "./scripts";
  ...
  return (
    <>
      <Container>
        ...
        <button onClick={showContainerNewComment}>adicionar comentário</button>

        <NewComment id="newComment" pathname={"new-comment?id=" + data.id} />
          ...
          data.comments.map((comment, commentIndex) => (
            <CommentContainer>
              ...
              >
                  ...
                  {comment?.ip
                    ? "#" +
                      ...
                      comment.ip
                        ...
                        .replace(/[ -.:]/g, "")
                        .slice(
                          comment.ip.toString().replace(/[ -.:]/g, "").length -
                            2,
                          comment.ip.toString().replace(/[ -.:]/g, "").length
                        )
                    : "#" + comment.id + ".00"}
                ...
                <p id="date">{newDate(comment.timestamp)}</p>
                <hr />
